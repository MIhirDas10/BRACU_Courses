...
{
       0,    39,    39,    48,    55,    64,    71,    81,    80,   102,
     101,   126,   135,   144,   155,   169,   168,   199,   198,   230,
     246,   253,   260,   267,   276,   286,   297,   308,   323,   330,
     339,   346,   354,   361,   368,   375,   382,   389,   396,   403,
     412,   419,   428,   436,   445,   452,   461,   468,   477,   484,
     493,   501,   510,   518,   528,   535,   542,   550,   557,   564,
     586,   593,   600,   607,   614,   623,   631,   639,   646
};
#endif
...
#line 231 "22299480_22101811.y"
    {
			...
			for(int i=0; i<(int)decl_list.size(); i++)
			{
				...
				if(!st->insert(decl_list[i])) delete decl_list[i];
			}
			decl_list.clear();
...
#line 565 "22299480_22101811.y"
    {
		...
		{
			arg_count = 1;
			for(int i=0; i<(int)(yyvsp[(3) - (4)])->getname().length(); i++)
			{
				if((yyvsp[(3) - (4)])->getname()[i] == ',') arg_count++;
			}
		}

		for(int i=1; i<=arg_count; i++)
		{
			outlog<<"At line no: "<<lines<<" argument "<<i<<" type mismatch in function call: "<<(yyvsp[(1) - (4)])->getname()<<endl<<endl;
		}
